...
Rust asks Lean semantic questions through a narrow, stable JSONL worker protocol. Rust must not inspect Lean
expressions, recompute semantic fingerprints from pretty-printed types, or let SQLite storage details leak into audit,
ranking, or reporting code.
...
The current Python tool proved the workflow: extract Lean declarations, compare local declarations against local and
external indexes such as mathlib, and turn confirmed duplication into cleanup hints. It is useful, but it is not the
right production boundary. Python owns too much semantic policy, cache behavior is scattered through audit code, and
...
- worker process lifecycle and JSONL protocol handling;
...
- worker command names outside the Rust worker runtime and protocol types;
...
- `doctor`, `index`, `index-mathlib`, `audit`, and `show` workflows;
- private/public filtering and optional direct or named import comparison;
- mathlib and other external-index comparison;
- text and JSON reports;
- progress and profile output that never corrupts JSON;
- read-only replacement hints with target declaration, import status, and bounded caller references.

Existing Python design mistakes to remove:

- Python-side semantic policy over Lean statements;
- source parsing as a fallback for facts Lean should own;
- storage details leaking into audit and ranking logic;
...
typed protocol records and index handles. This design is deeper because Rust has a smaller semantic interface, Lean
...
2. **Stable JSONL protocol.**
   The protocol carries schema-versioned requests, responses, progress events, and structured errors. Declaration rows,
   feature rows, and probe results are caller-facing facts. Cache layout, SQLite tables, and report formatting are not
   protocol facts. Prompt 02 owns the executable schema.
...
- **Worker protocol.** The protocol hides process framing, schema compatibility, stderr policy, progress delivery,
  structured error mapping, and worker-version validation from audit logic.
...
- no Rust/Python recomputation of Lean semantic facts from pretty text;
- no Python-style pass-through facade modules;
...
Lean/Rust FFI as the primary route. The production path is a batched subprocess worker over stable JSONL unless prompt
19 proves, with measurements, that an FFI migration is worth the safety and maintenance cost.

## Migration Map

The prompt sequence is the migration plan. The current Python implementation is behavior reference until parity and
switchover are complete; it is not the target architecture.

| Prompt | Responsibility |
| --- | --- |
| 02 | Specify the protocol/schema, cache keys, and worker error model. |
...
| 16 | Prove Python parity, switch docs/install to the production binary, and remove superseded Python only when safe. |
...
profile/progress data, preserve known exact and inspected cleanup findings, and keep broad-head or generated noise out
