...
toolchain is `leanprover/lean4:v4.33.0-rc1` (other 4.x untested).
...
# Provision the per-toolchain worker (worker-child binary + LeanDup capability dylib).
...
The worker integration tests build the worker-child and the `LeanDup` capability on first run (memoized on disk under
the install dir).
...
| `capability-source` | the vendored `LeanDup` Lean source + the runtime `build_capability_into` that produces the capability dylib/manifest on the user's machine (lifted from the deleted `worker/build.rs`) | install-dir layout, sidecar schema, worker transport |
| `project` | `workspace.rs` (Lake discovery, module roots, source enumeration), `mathlib.rs` (project-pinned mathlib contract) | — |
| `worker` | Rust side of the worker protocol; pool capability lifecycle (`engine/`: `WorkerEngine`/`PoolEngine`/`LeanDupCapabilityRuntime`); per-workspace toolchain resolution + install-dir/sidecar (`toolchain.rs`), substrate facts, error mapping | Lean `Expr` constructors, pretty text, private worker batching, capability symbol names, child ABI |
...
in sync (the vendored copy is what survives a crates.io unpack). `Capability` exposes five capability command exports
loaded by the pool: `version` (json command) and the streaming `extract`, `features`, `index`, `probe`. `doctor`-style
health is composed Rust-side from `version` plus workspace/cache checks; worker substrate facts come from the pool
handshake. See `docs/architecture/validation/worker-migration-validation.md`.
