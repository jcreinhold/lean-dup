...
declaration pairs, reusing the shared session environment when the module
signature matches the previous command (see `LeanDup.Extract.sessionEnv`).
  ...
  | Except.ok pairs =>
      ...
      | Except.ok options =>
          if modules.isEmpty then
            ...
            match ← LeanDup.Extract.sessionEnv modules options initializeSearchPath with
            | Except.error err => pure <| Except.error (fromExtractError err)
