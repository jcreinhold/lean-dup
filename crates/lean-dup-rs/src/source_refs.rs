...
pub(crate) struct SourceFacts {
    pub(crate) declarations: BTreeMap<String, DeclarationSourceFact>,
    pub(crate) files: BTreeMap<PathBuf, SourceFileFact>,
...
impl SourceFacts {
    ...
    pub(crate) fn declaration(&self, declaration_id: &str) -> Option<&DeclarationSourceFact> {
        self.declarations.get(declaration_id)
    }

    pub(crate) fn caller_count(&self, declaration_id: &str) -> usize {
        self.declaration(declaration_id)
            .map(|fact| fact.references.len())
            .unwrap_or(0)
    }

    pub(crate) fn source_fingerprint(&self, declaration_id: &str) -> Option<&str> {
        self.declaration(declaration_id)
...
impl<'a> SourceFactInput<'a> {
    pub(crate) fn new(declarations: &'a [HydratedDeclaration]) -> Self {
        Self {
            declarations,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub(crate) struct DeclarationSourceFact {
...
pub(crate) fn collect_source_facts(input: SourceFactInput<'_>) -> SourceFacts {
