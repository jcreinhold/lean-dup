...
pub(crate) struct BaselineSnapshot {
    ...
    pub(crate) groups: Vec<BaselineGroup>,
}
...
pub(crate) struct BaselineDiff {
    ...
    pub(crate) appeared: Vec<BaselineGroup>,
    pub(crate) disappeared: Vec<BaselineGroup>,
    pub(crate) changed: Vec<BaselineChange>,
}

/// One group whose stable identity survived but evidence changed.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub(crate) struct BaselineChange {
    pub(crate) id: String,
    pub(crate) before: BaselineGroup,
    pub(crate) after: BaselineGroup,
}

/// Build the storage-independent snapshot used by save-baseline and diff.
pub(crate) fn snapshot(review: &RankedReview, workspace_fingerprint: String) -> BaselineSnapshot {
    BaselineSnapshot {
...
pub(crate) fn save(cache_root: &Path, name: &str, snapshot: &BaselineSnapshot) -> Result<PathBuf> {
...
pub(crate) fn load(cache_root: &Path, name: &str) -> Result<(PathBuf, BaselineSnapshot)> {
    let path = baseline_path(cache_root, name)?;
