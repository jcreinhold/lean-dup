...
use crate::worker::ModuleDescriptor;
...
pub(crate) enum ComparisonEvidenceMode {
    Static,
...
pub(crate) struct ComparisonProvenanceReport {
...
pub(crate) struct ComparisonEvidencePolicy {
...
impl ComparisonEvidencePolicy {
    #[cfg(test)]
    pub(crate) fn for_origin(origin: impl Into<String>, mode: ComparisonEvidenceMode) -> Self {
    ...
    pub(crate) fn evidence_mode(&self, origin: &str) -> ComparisonEvidenceMode {
        self.origins
            .get(origin)
    ...
    pub(crate) fn requires_semantic_evidence(&self, origin: &str) -> bool {
        self.evidence_mode(origin) == ComparisonEvidenceMode::ProofGrade
    }

    pub(crate) fn probe_module(&self, origin: &str, module: &str) -> Option<ModuleDescriptor> {
...
pub(crate) struct ComparisonProvenance {
    pub(crate) reports: Vec<ComparisonProvenanceReport>,
    #[serde(skip)]
    pub(crate) policy: ComparisonEvidencePolicy,
}
...
) -> crate::error::Result<ComparisonProvenance> {
