...
pub(crate) fn render_metrics(metrics: &EvaluationMetrics) -> String {
    ...
    [
        "suite\trecall@1\trecall@5\trecall@10\tqueue_precision\thard_negatives\tcandidates\tindex_load_ms\tretrieval_ms\tprobe_ms\ttotal_ms\tpeak_mem",
        &format!(
            "{suite}\t{recall_1}\t{recall_5}\t{recall_10}\t{precision}\t{hard_negatives}\t{candidates}\t{index_load_ms}\t{retrieval_ms}\t{probe_ms}\t{total_ms}\t{peak_memory}",
            suite = metrics.suite,
