...
pub(crate) struct SearchStageMetrics {
    pub(crate) candidate_generation_recall: CountMetric,
    pub(crate) top_k_recall_before_final_ranking: Vec<RecallAtK>,
    pub(crate) ranked_recall: Vec<RecallAtK>,
    pub(crate) visible_queue_precision: CountMetric,
    pub(crate) hard_negative_survival: HardNegativeSurvival,
    pub(crate) candidate_count_by_origin: BTreeMap<String, usize>,
    pub(crate) candidate_count_by_feature_family: BTreeMap<String, usize>,
    pub(crate) semantic_verification: SemanticVerificationStageMetrics,
}

#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize)]
pub(crate) struct HardNegativeSurvival {
    pub(crate) candidate_generation: CountMetric,
    pub(crate) top_k: Vec<CountAtK>,
    pub(crate) visible_queue: CountMetric,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub(crate) struct CountAtK {
    pub(crate) k: usize,
...
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize)]
pub(crate) struct SemanticVerificationStageMetrics {
...
pub(crate) fn score(labels: &GoldLabels, observed: &ObservedRun, k_values: &[usize]) -> SearchStageMetrics {
...
pub(crate) fn aggregate(_suite: &str, runs: &[&SearchStageMetrics]) -> SearchStageMetrics {
    let k_values = {
        let mut values = runs
            .iter()
            .flat_map(|metrics| {
