...
pub(crate) struct GoldLabels {
    ...
    pub(crate) typed_pairs: Vec<TypedGoldLabel>,
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct TypedGoldLabel {
    pub(crate) pair: GoldPair,
    pub(crate) polarity: LabelPolarity,
    pub(crate) match_class: MatchClass,
    pub(crate) expected_stage_visibility: ExpectedStageVisibility,
    pub(crate) adjudication_source: AdjudicationSource,
    pub(crate) confidence: LabelConfidence,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub(crate) enum ExpectedStageVisibility {
    Candidate,
    Ranked,
    Visible,
    Hidden,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub(crate) enum AdjudicationSource {
    FixtureIntent,
    ManualInspection,
    Prompt27Evidence,
    PythonEraRegression,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub(crate) enum LabelConfidence {
    High,
    Medium,
    Low,
}
