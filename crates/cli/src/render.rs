...
use lean_dup_index::cache_lifecycle::CacheCleanupReport;
use lean_dup_report::Result;
use lean_dup_report::perf::{self, CostClass};
use lean_dup_report::progress::{Reporter, format_progress_event};
use lean_dup_search::ranking::{RankedGroup, ReviewAction, ReviewEvidenceMode, ReviewPriority, ReviewRelation};
use lean_dup_search::report_contract::GroupExplanation;
    ...
    perf::measure_result(CostClass::Reporting, "report.render", || {
        ...
        if let Some(path) = outcome.output_path {
            if let Some(parent) = path.parent() {
                std::fs::create_dir_all(parent).map_err(|source| lean_dup_report::Error::Io {
                    message: "could not create CLI output directory",
                    path: parent.to_path_buf(),
                    source,
                })?;
            }
            std::fs::write(&path, format!("{rendered}\n")).map_err(|source| lean_dup_report::Error::Io {
...
fn render_text(report: &Report) -> String {
    match report {
        ...
        Report::Eval(report) => lean_dup_eval::eval::table::render_metrics(&report.metrics),
