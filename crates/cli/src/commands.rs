...
use lean_dup_eval::eval::{EvalRequest, EvaluationReport};
use lean_dup_index::cache::{self, CacheFacts};
use lean_dup_index::cache_lifecycle::{CacheCleanupReport, CacheDiagnostics, CleanupPolicy};
use lean_dup_index::external_provenance::{ComparisonProvenance, ComparisonProvenanceReport};
...
use lean_dup_search::baseline::{self, BaselineDiff};
use lean_dup_search::ranking::{
    RankedGroup, RankedReview, RankingInput, RankingProfile, ReviewFilter, ReviewPriority as RankedPriority,
    rank_candidates,
};
use lean_dup_search::replacement_hints::{
    ReplacementHintProfile, attach_replacement_hints, reference_declarations_for_hints,
};
use lean_dup_search::report_contract::{AuditExplanations, GroupExplanation};
use lean_dup_search::retrieval::{RetrievalDiagnostics, retrieve_candidates};
use lean_dup_search::semantic_verification::{
...
use lean_dup_search::source_refs::{SourceFactInput, collect_source_facts};
...
pub struct AuditReport {
    ...
    pub show_noise: bool,
    pub min_priority: RankedPriority,
...
fn doctor(args: DoctorArgs, reporter: &mut Reporter) -> Result<DoctorReport> {
    ...
    let cache_diagnostics =
        lean_dup_index::cache_lifecycle::diagnose_cache(foundation.cache.root.clone(), &[current_index], &store)?;
...
fn cache_cleanup(args: CacheCleanupArgs, reporter: &mut Reporter) -> Result<CacheCleanupReport> {
    ...
    let expected_entries = if let Some(workspace_root) = args.workspace {
        ...
        let worker_version = version_call
            ...
            .into_iter()
            .next()
            .ok_or_else(|| lean_dup_report::Error::Index {
    ...
    lean_dup_index::cache_lifecycle::cleanup_cache(
        cache_root,
        &expected_entries,
...
fn audit(args: AuditArgs, reporter: &mut Reporter) -> Result<AuditReport> {
    ...
    let explanations = lean_dup_search::report_contract::explain_audit(
        &computation.review,
        &visible_groups,
        filter,
    ...
    Ok(AuditReport {
        report_schema_version: lean_dup_search::report_contract::REPORT_SCHEMA_VERSION,
        status: "ok",
...
fn open_compare_indexes(
    ...
    let provenance = lean_dup_index::external_provenance::resolve(&indexes, project_workspace)?;
    Ok(CompareIndexes { indexes, provenance })
}

fn source_fact_declarations(
    workspace_rows: &[lean_dup_index::index::HydratedDeclaration],
    candidate_sets: &[lean_dup_search::retrieval::CandidateSet],
    compare_mathlib: bool,
    review_profile: ReviewProfile,
    show_noise: bool,
) -> Vec<lean_dup_index::index::HydratedDeclaration> {
...
fn show(args: ShowArgs, reporter: &mut Reporter) -> Result<ShowReport> {
    ...
    let explanation = lean_dup_search::report_contract::explain_group(&group, filter);
    Ok(ShowReport {
        status: "ok",
        requested_workspace: computation.foundation.workspace.requested_root,
        lake_root: computation.foundation.workspace.root,
